...
bdv.radialHistogram = function module() {
    ...
    function chart(_selection) {
        ...
        function mouseover(d) {
            centerText.text('Total Delays: ' + d.value);
        }
...
zurichMap.on('stopsEnd', function(){
    zurichDataManager.on('dataLoading', function () {
        ...
        sweetAlert("Please Wait!", "Data Loading", "warning");
    });
    zurichDataManager.on('dataReady', function () {
        sweetAlert("Data Ready!", "you can now start data exploration by brushing on the map", "success");
    });

    //Show stop information
    zurichMap.on('hover', function(d){
        d3.select('#stop-name')
            .text('stopName: ' + d.properties.stopName + ' stopID: ' + d.properties.stopId);
